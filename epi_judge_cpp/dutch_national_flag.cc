...
void DutchFlagPartition(int pivot_index, vector<Color> *A_ptr)
{
    ...
    int next_less_index = 0;
    int next_equal_index = 0;
    int next_greater_index = n - 1;

    while (next_equal_index <= next_greater_index) {
        Color curr = A[next_equal_index];
        
        if (curr == pivot) {
            next_equal_index++;
        } else if (curr < pivot) {
            std::swap(A[next_less_index], A[next_equal_index]);
            next_less_index++;
            next_equal_index++;
        } else {
            std::swap(A[next_equal_index], A[next_greater_index]);
            next_greater_index--;
        }
    }
